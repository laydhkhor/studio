      ...
      <div className="grid gap-6 max-w-4xl">
        <Card className="shadow-xl border-none overflow-hidden">
          <CardHeader className="bg-primary/5 border-b">
            ...
            <CardDescription>Predefined categories for consultation reports.</CardDescription>
          </CardHeader>
          <CardContent className="pt-8 space-y-8">
            <div className="flex items-end gap-4 max-w-md">
              <div className="flex-1 space-y-2">
                <Label className="text-xs uppercase font-bold text-slate-400">Add New Category</Label>
                <Input placeholder="e.g., Cardiology" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} />
              </div>
              <Button onClick={handleAddCategory} className="font-bold" disabled={isSaving || !newCategory.trim()}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {categories.map((cat: string) => (
                <div key={cat} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleRemoveCategory(cat)} className="text-slate-400 hover:text-red-600">
