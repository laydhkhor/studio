...
export default function AdminLayout({
  ...
  React.useEffect(() => {
    ...
    if (!isUserLoading && !isRoleLoading) {
      ...
      } else if (user.email !== 'devilcry160@gmail.com' && userData?.role !== 'doctor' && userData?.role !== 'dev') {
  ...
  const isAuthorized = user?.email === 'devilcry160@gmail.com' || userData?.role === 'doctor' || userData?.role === 'dev';

  if (!user || !isAuthorized) {
    return null;
  }

  const allNavItems = [...adminNavItems, ...adminSettingsNav];
  const currentPath = allNavItems.find(item => item.href === pathname)?.label || 'Overview';
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64 shadow-2xl rounded-xl border-slate-200 p-2">
                ...
                <DropdownMenuItem asChild className="rounded-lg cursor-pointer py-2 px-3">
                  <Link href="/admin-dashboard/settings" className="flex items-center gap-2">
